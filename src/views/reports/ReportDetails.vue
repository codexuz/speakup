<template>
  <ion-page>
    ...
    <ion-content class="ion-padding">
      ...
      <!-- Error State -->
      <div v-if="!loading && error" class="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <ion-icon :icon="alertCircleOutline" class="text-red-500 text-6xl mb-4"></ion-icon>
        ...
        <ion-button @click="$router.push('/reports')" fill="clear" color="medium" class="mt-2">
          Back to Reports
        </ion-button>
      </div>

      <!-- No Data State -->
      <div v-if="!loading && !error && !response" class="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <ion-icon :icon="documentOutline" class="text-gray-400 text-6xl mb-4"></ion-icon>
        <h2 class="text-2xl font-bold text-gray-800 mb-2">No Data Available</h2>
        <p class="text-gray-600 text-center mb-6">We couldn't find any assessment data for this report.</p>
      ...
      <section v-if="loading" class="mt-6">
        <ion-skeleton-text :animated="true" class="mb-4 rounded-xl w-48 h-8"></ion-skeleton-text>
        <ion-skeleton-text :animated="true" class="mb-6 rounded-xl w-full h-[200px]"></ion-skeleton-text>
        <ion-skeleton-text :animated="true" class="mb-6 rounded-xl w-full h-[200px]"></ion-skeleton-text>
        <ion-skeleton-text :animated="true" class="mb-6 rounded-xl w-full h-[200px]"></ion-skeleton-text>
        <ion-skeleton-text :animated="true" class="mb-6 rounded-xl w-full h-[200px]"></ion-skeleton-text>
      </section>
      ...
      <section v-if="!loading && !error && response" class="mt-6">
        <div class="mx-4 flex items-center gap-x-6">
          <span><b>Words: {{ totalWords }}</b></span>
        </div>
        ...
        <ion-skeleton-text v-if="loading" :animated="true" class="mb-6 rounded-xl w-full h-[200px]"></ion-skeleton-text>
        <ion-skeleton-text v-if="loading" :animated="true" class="mb-6 rounded-xl w-full h-[200px]"></ion-skeleton-text>

        <ion-card mode="md" class="rounded-xl">
          <ion-card-content>
            <ion-card-subtitle>
              <span class="text-slate-900 poppins-bold text-lg block">Feedback</span>
              <span v-if="earnedLevelDescript">{{ earnedLevelDescript }}</span>
              <span v-else class="text-gray-500 italic">No feedback available yet</span>
            </ion-card-subtitle>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card class="rounded-xl">
          <ion-card-content class="p-5">
            <ion-list>
              <ion-card-title class="text-lg">Speaking Rate (Words Per Minute)</ion-card-title>
              <ion-item lines="none">
                <button class="flex items-center gap-x-3">
                  <svg class="bg-blue-600 p-1 rounded-full text-white" xmlns="http://www.w3.org/2000/svg" width="40"
                ...
                <ion-label>
                  <span class="px-3 text-3xl">{{ transcriptionDetails?.speaking_rate || 0 }} </span>
                </ion-label>
              </ion-item>
            </ion-list>
            <div class="flex items-center gap-x-3 px-5 pt-4">
              <p>Common speaking rate for native speakers in the US <span
                  class="text-3xl font-medium text-gray-700 px-1">90-150</span></p>
            </div>

            <RangeSlider :level="Number(transcriptionDetails?.speaking_rate || 0)" />
          </ion-card-content>
        </ion-card>


        <ion-list lines="none" class="p-5 mb-8 mx-3 rounded-xl" v-if="hasVocabularyData">
          <ion-list-header>
            <ion-label class="p-3">Vocabulary By Level</ion-label>
          </ion-list-header>
          <ion-item>
            <ion-badge class="p-2" color="danger">{{ levelA1 }}%</ion-badge>
            <ion-label class="mx-3">- Beginner (A1)</ion-label>
          </ion-item>
          <ion-item>
            <ion-badge class="p-2" color="warning">{{ levelA2 }}%</ion-badge>
            <ion-label class="mx-3">- Elementary (A2)</ion-label>
          </ion-item>
          <ion-item>
            <ion-badge class="p-2" color="light">{{ levelB1 }}%</ion-badge>
            <ion-label class="mx-3">- Intermediate (B1)</ion-label>
          </ion-item>
          <ion-item>
            <ion-badge class="p-2" color="success">{{ levelB2 }}%</ion-badge>
            <ion-label class="mx-3">- Upper-intermediate (B2)</ion-label>
          </ion-item>
          <ion-item>
            <ion-badge class="p-2" color="secondary">{{ levelC1 }}%</ion-badge>
        ...
        <ion-card class="rounded-xl" v-if="!hasVocabularyData">
          <ion-card-content class="text-center py-8">
            <ion-icon :icon="libraryOutline" class="text-gray-400 text-5xl mb-3"></ion-icon>
            <p class="text-gray-600">Vocabulary analysis is not available for this assessment yet.</p>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card class="rounded-xl" v-if="audioUrl && isAudioValid">
          <ion-card-header>
            <ion-card-title class="text-gray-800 flex items-center justify-between">
              ...
              <ion-button 
                ...
                v-if="wavesurfer"
              >
                <ion-icon slot="icon-only" :icon="isPlaying ? pauseOutline : playOutline"></ion-icon>
              </ion-button>
            </ion-card-title>
          </ion-card-header>
          <ion-card-content>
        ...
        <!-- Audio Error Message -->
        <ion-card class="rounded-xl" v-if="audioUrl && !isAudioValid && audioLoadError">
          <ion-card-content class="text-center py-8">
            <ion-icon :icon="alertCircleOutline" class="text-red-500 text-5xl mb-3"></ion-icon>
            <h3 class="text-lg font-semibold text-gray-800 mb-2">Audio Unavailable</h3>
            <p class="text-gray-600">{{ audioLoadError }}</p>
            <p class="text-sm text-gray-500 mt-2">The audio file may be corrupted or no longer available.</p>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card class="rounded-xl" v-if="transcriptionDetails">
          <ion-card-content>
            <ion-card-title class="text-gray-800 mb-3">Transcription Details</ion-card-title>
            <ion-list>
              <ion-item>
                <ion-label>Total Duration</ion-label>
                <ion-label slot="end">{{ formatDuration(transcriptionDetails.total_duration) }}</ion-label>
              </ion-item>
              <ion-item>
                <ion-label>Word Count</ion-label>
                <ion-label slot="end">{{ transcriptionDetails.word_count }}</ion-label>
              </ion-item>
              <ion-item>
                <ion-label>Speaking Rate</ion-label>
                <ion-label slot="end">{{ transcriptionDetails.speaking_rate }} WPM</ion-label>
              </ion-item>
              <ion-item>
                <ion-label>Filler Words</ion-label>
                <ion-label slot="end">{{ transcriptionDetails.filler_word_count }}</ion-label>
              </ion-item>
              <ion-item v-if="pauseAnalysis && pauseAnalysis.pause_count">
                <ion-label>Pauses</ion-label>
                <ion-label slot="end">{{ pauseAnalysis.pause_count || 0 }}</ion-label>
              </ion-item>
              <ion-item v-if="pauseAnalysis && pauseAnalysis.average_pause_duration">
                <ion-label>Average Pause Duration</ion-label>
                <ion-label slot="end">{{ (pauseAnalysis.average_pause_duration || 0).toFixed(2) }}s</ion-label>
              </ion-item>
              <ion-item v-if="pauseAnalysis && pauseAnalysis.longest_pause">
                <ion-label>Longest Pause</ion-label>
                <ion-label slot="end">{{ (pauseAnalysis.longest_pause || 0).toFixed(2) }}s</ion-label>
              </ion-item>
              <ion-item>
                <ion-label>Confidence Score</ion-label>
                <ion-label slot="end">{{ ((transcriptionDetails.confidence_score || 0) * 100).toFixed(1) }}%</ion-label>
              </ion-item>
            </ion-list>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card class="rounded-xl" v-if="transcript">
          <ion-card-header>
            <ion-card-title class="text-white flex items-center gap-2">
              <ion-icon :icon="documentTextOutline"></ion-icon>
              Your Response
            </ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <p class="text-slate-800 leading-relaxed whitespace-pre-line">{{ transcript }}</p>
          </ion-card-content>
        </ion-card>

        <!-- Questions Asked -->
        <ion-card class="rounded-xl" v-if="testQuestions && Object.keys(testQuestions).length > 0">
          <ion-card-content>
            <ion-card-title class="text-gray-800 flex items-center mb-3 gap-2">
              Questions Asked
            </ion-card-title>
            <div v-for="(questions, part) in testQuestions" :key="part" class="mb-4">
              <h3 class="text-lg font-semibold text-gray-800 mb-2">Part {{ part }}</h3>
...
<script setup>
import { ref, useTemplateRef, onMounted, onBeforeUnmount, computed, watch } from "vue";
...
const grammarDetailsFeed = computed(() => {
  const grammarFeedback = response.value?.response?.assessment?.aiAssessment?.grammarFeedback || '';
  return {
...
const suggestions = computed(() => {
  const feedback = response.value?.response?.assessment?.aiAssessment?.feedback || '';
  const fluencyFb = response.value?.response?.assessment?.aiAssessment?.fluencyFeedback || '';
  const grammarFb = response.value?.response?.assessment?.aiAssessment?.grammarFeedback || '';
  const vocabFb = response.value?.response?.assessment?.aiAssessment?.vocabularyFeedback || '';
  const pronFb = response.value?.response?.assessment?.aiAssessment?.pronunciationFeedback || '';

  return `${feedback}\n\nKey areas to focus on:\n- Fluency: ${fluencyFb}\n- Grammar: ${grammarFb}\n- Vocabulary: ${vocabFb}\n- Pronunciation: ${pronFb}`;
});

const wordsLevel = computed(() => {
  return response.value?.response?.assessment?.wpm || response.value?.response?.transcription_details?.speaking_rate || 0;
});

const rareWord = computed(() => {
  const levels = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels;
...
const levelA1 = computed(() => {
  const level = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels?.A1;
  return level ? level.toFixed(1) : 0;
});

const levelA2 = computed(() => {
  const level = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels?.A2;
  return level ? level.toFixed(1) : 0;
});

const levelB1 = computed(() => {
  const level = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels?.B1;
  return level ? level.toFixed(1) : 0;
});

const levelB2 = computed(() => {
  const level = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels?.B2;
  return level ? level.toFixed(1) : 0;
});

const levelC1 = computed(() => {
  const level = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels?.C1;
  return level ? level.toFixed(1) : 0;
});

const levelC2 = computed(() => {
  const level = response.value?.response?.assessment?.vocabularyLevel?.meta?.levels?.C2;
  return level ? level.toFixed(1) : 0;
});

// Additional computed properties for new sections
const audioUrl = computed(() => {
  return response.value?.audio_url || null;
});

const transcript = computed(() => {
  return response.value?.transcript || response.value?.response?.assessment?.transcript || '';
});
