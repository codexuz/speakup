...
export async function stopRecording() {
  try {
    ...
    if (!isCurrentlyRecording) {
      ...
      return new Blob([], { type: "audio/mp4" });
    }

    // Check if there's an active recording before trying to stop
    const hasPermission = await VoiceRecorder.hasAudioRecordingPermission();
    if (!hasPermission.value) {
      console.warn("No recording permission, cannot stop recording");
      isCurrentlyRecording = false;
      throw new Error("No recording permission");
    }

    const recording = await VoiceRecorder.stopRecording();
    isCurrentlyRecording = false; // Reset state after stopping
    ...
    isCurrentlyRecording = false; // Reset state on error
    
    // Handle specific "RECORDING_HAS_NOT_STARTED" error gracefully
    if (error.message && error.message.includes("RECORDING_HAS_NOT_STARTED")) {
      console.warn("Attempted to stop recording that hasn't started or already stopped");
      // Return a minimal empty blob to avoid breaking the flow
      return new Blob([], { type: "audio/mp4" });
    }
    
    console.error("Failed to stop recording:", error);
    throw error; // Re-throw other errors to handle in the calling function
  }
}
